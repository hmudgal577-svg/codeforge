...
class ASTAnalyzer {
     ...
     */
    computeComplexity(code, lines, functions) {
        ...
        const mi = Math.max(0, Math.min(100, 171 - 5.2 * Math.log(Math.max(1, codeLines)) -
            0.23 * avgComplexity -
     ...
     */
    buildDependencyGraph(functions, imports, classes) {
        const nodes = [];
        const edges = [];
        // Add function nodes
     ...
     */
    detectVulnerabilities(code, lines, language) {
        const vulns = [];
        const patterns = [
    ...
    findBlockEnd(lines, start, language) {
        if (language === 'python') {
    ...
    extractParams(line, language) {
        const match = line.match(/\(([^)]*)\)/);
        if (!match)
            return [];
        return match[1].split(',').map(p => p.trim()).filter(Boolean);
    }
    extractFunctionCalls(body, language) {
        const calls = new Set();
    ...
    cognitiveComplexity(code, language) {
        let complexity = 0;
        const lines = code.split('\n');
        let nesting = 0;
    ...
    computeMetrics(lines, functions, classes, imports) {
        const commentLines = lines.filter(l => this.isComment(l.trim())).length;
        const blankLines = lines.filter(l => l.trim() === '').length;
